...
* `bufferHeight`: how much content we should load offscreen, before the user scrolls there. If this option isn't passed, we default to double your selector's height. You can pass a fixed number, but I suggest passing a function that takes the selector area's height (the view's height, not the content height), e.g. `function(height) { return height * 2.71828; }`. 
* `scrollInterval`: in milliseconds, how often we should handle the scrolling event that checks if we need to load new content, defaults to `50`.
...
* `positionCallback`: called whenever scrolling is handled (depends on `scrollInterval`), receiving three arguments: the percentage position we are in (as a float), the absolute position we are in, and the total height of loaded content so far.
